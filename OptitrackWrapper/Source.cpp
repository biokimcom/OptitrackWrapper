/*********************************************************************
 * \Defines the exported fnctions for the DLL for LabVIEW
 * \Seung-Jae Kim, 2024
 * \California Baptist University, Bioengineering
 *********************************************************************/
...
//void OutputFrameQueueToConsole(std::vector<float>& arr);
int OutputFrameQueueToConsole(float* arr, int32_t nRBs, int32_t nMar, size_t arrSize);
...
bool initNatNetClient();
int ConnetToMotive();
bool closeConnect();
void thread_sleep(int ms);
void updateArray(float* arr, int size);
void updateArray2D(float** arr, int rows, int clos);

int main2(int argc, char* argv[]) {

    float array[1000] = { 99 };
    //float* array = new float[1000] { 99 };
    int nDQ = 0;
    bool a1 = initNatNetClient();

    bool a2 = ConnetToMotive();
    ...
    while (bRunning)
    {
        ...
        nDQ = OutputFrameQueueToConsole(array, 2, 8, 100);
...
void updateArray(float* arr, int size) {
    ...
    float* arr2 = new float[1000] { 0 };

    for (int i = 0; i < size-2; ++i) {
        arr2[i] = 5.0;  // Increment each element by 1
        arr[i] = arr2[i];
    }
...
void updateArray2D(float** arr, int rows, int clos) {
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < clos; ++j) {
                arr[i][j] += 1.0;  // Increment each element by 1
        }
    }
}
...
bool initNatNetClient() {
    ...
    const sNatNetDiscoveredServer& discoveredServer = g_discoveredServers[a];
...
int OutputFrameQueueToConsole(float* arr, int32_t nRBs, int32_t nMar, size_t arrSize)
{
    ...
    //MocapFrameWrapper f = displayQueue.back();
    ...
    for (MocapFrameWrapper f : displayQueue)
    {
        ...
        */
        ...
        for (i = 0; i < data->nRigidBodies; i++)
        {
            ...
            //arr[nidx] = data->RigidBodies[i].x;
            //nidx = nidx + 1;
            //arr[nidx] = data->RigidBodies[i].y;
            //nidx = nidx + 1;
            //arr[nidx] = data->RigidBodies[i].z;
            //nidx = nidx + 1;
